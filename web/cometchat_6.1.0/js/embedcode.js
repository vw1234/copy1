...
function addEmbedIframe(iframeObj,callbackfn) {
	if(iframeObj.module=="synergy"){
		...
		if(jqcc('link[href*="'+baseurl+'cometchatcss.php"]').length<=0 && cbfn!='desktop') {
			jqcc('head').append('<link type="text/css" id="cc_link" href="'+baseurl+'cometchatcss.php?cc_theme=synergy" rel="stylesheet" charset="utf-8">');
			addScript(baseurl+'cometchatjs.php');
		}else{
			jqcc('link[href*="'+baseurl+'cometchatcss.php"]').attr('href',baseurl+'cometchatcss.php?cc_theme=synergy');
		}
	}
