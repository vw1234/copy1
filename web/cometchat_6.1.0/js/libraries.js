...
function loadCCPopup(url,name,properties,width,height,title,force,allowmaximize,allowresize,allowpopout,windowmode) {
	...
	if(typeof(windowmode) != "undefined" && windowmode == 1 && typeof url != "undefined" && typeof name != "undefined") {
		...
		if (jqcc('#cometchat_container_'+name).length > 0) {
			...
			setTimeout(function() {
				cc_zindex += 1;
				jqcc('#cometchat_container_'+name).css('z-index',300001+cc_zindex);
			}, 100);
			return;
		}
		...
		cometchat_container.click(function() {
			cc_zindex += 1;
			jqcc(this).css('z-index',300001+cc_zindex);
		});
	}
}
...
function dragStart(a,b){
	cc_zindex += 1;jqcc('#'+b).css('z-index',300001+cc_zindex);
