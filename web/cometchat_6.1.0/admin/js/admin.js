...
function modules_removemodule(id,custom) {
	...
	if (answer) {
		...
		if (id == 'chatrooms') {
			var synergychatroom = confirm('Do you also want to disable Chatrooms from Synergy theme and display only one-on-on chat?');
			if(synergychatroom) {
				$.post('?module=dashboard&action=loadthemetype&type=theme&name=synergy&updatesettings=true', {'enableType': '2'}, function(data) {});
			}
		}
...
function create_new_colorscheme(){
        location.href = '?module=themes&action=clonecolor&theme=glass&ts='+ts;
}
...
function themetype_configmodule(id) {
	window.open('?module=dashboard&action=loadthemetype&type=theme&name='+id+'&embedcode=0&ts='+ts,'external','width=400,height=500,resizable=1,scrollbars=1');
}

function themetype_embedcode(id) {
	window.open('?module=dashboard&action=themeembedcodesettings&type=theme&name='+id+'&embedcode=1&ts='+ts,'external','width=400,height=500,resizable=1,scrollbars=1');
}
...
function language_updatelanguage(element) {
	...
	if(element[0].type == 'radio'){
		...
		lang_key = element.siblings('textarea').attr('lang_key');
		lang_text = element.siblings('textarea').val();
		code = element.siblings('textarea').attr('code');
		type = element.siblings('textarea').attr('addontype');
		name = element.siblings('textarea').attr('addonname');
	}
...
function language_importlanguage(id) {
	...
	if (answer) {
		$("#over").show();
		$.getJSON('//www.cometchat.com/software/getlanguage/?v=6&callback=?', {id: id}, function(data) {
			if (data) {
...
function language_getlanguages() {
	$.getJSON('//www.cometchat.com/software/getlanguages/?v=6&callback=?', {}, function(data) {
		if (data) {
			var html = '';
...
function embed_link(url,width,height) {
	...
	if(module[0]=='chatrooms') {
		embedlink = window.open('','embedlink','width=520,height=350,resizable=0,scrollbars=0');
	} else {
	...
	if(module[0]=='chatrooms'){
		if(mod[mod.length-1].indexOf('id=') > -1) {
			...
			var synergyembedscript = '<script src="'+baseUrl+'js.php?type=core&amp;name=embedcode" type="text/javascript"></script>\n<script>var iframeObj = {};iframeObj.module="chatrooms";iframeObj.src="'+baseUrl+'cometchat_popout.php?crid='+crid+'";iframeObj.width="'+width+'";iframeObj.height="'+height+'";if(typeof(addEmbedIframe)=="function"){addEmbedIframe(iframeObj);}</script>';
			embedlink.document.write('<strong style="font-family: arial,sans-serif; color: #333333;">1. For Classic Styling:</strong><textarea readonly style="width:500px;height:140px"><div id="cometchat_embed_chatrooms_container" style="display:inline-block; border:1px solid #CCCCCC;"></div>\n'+embedscript+'</textarea><br><br><strong style="font-family: arial,sans-serif; color: #333333;">2. For Synergy Styling:</strong><textarea readonly style="width:500px;height:140px"><div id="cometchat_embed_chatrooms_container" style="display:inline-block; border:1px solid #CCCCCC;"></div>\n'+synergyembedscript+'</textarea>');
		} else {
			var synergyembedscript = '<script src="'+baseUrl+'js.php?type=core&amp;name=embedcode" type="text/javascript"></script>\n<script>var iframeObj = {};iframeObj.module="chatrooms";iframeObj.style="min-height:420px;min-width:300px;";iframeObj.src="'+baseUrl+'cometchat_popout.php?chatroomsonly=1";iframeObj.width="'+width+'";iframeObj.height="'+height+'";if(typeof(addEmbedIframe)=="function"){addEmbedIframe(iframeObj);}</script>';
			embedlink.document.write('<strong style="font-family: arial,sans-serif; color: #333333;">1. For Classic Styling:</strong><textarea readonly style="width:500px;height:140px"><div id="cometchat_embed_chatrooms_container" style="display:inline-block; border:1px solid #CCCCCC;"></div>\n'+embedscript+'</textarea><br><br><strong style="font-family: arial,sans-serif; color: #333333;">2. For Synergy Styling:</strong><textarea readonly style="width:500px;height:140px"><div id="cometchat_embed_chatrooms_container" style="display:inline-block; border:1px solid #CCCCCC;"></div>\n'+synergyembedscript+'</textarea>');
		}
		...
		embedlink.document.write('<textarea readonly style="width:500px;height:70px"><iframe src="'+url+'" width="'+width+'" height="'+height+'" frameborder="1" id="cometchat_embed_'+module[0]+'" name="cometchat_'+module[0]+'_iframe"></iframe></textarea><script>window.resizeTo(520,125);</script>');
	}
