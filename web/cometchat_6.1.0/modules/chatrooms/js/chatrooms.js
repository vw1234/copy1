...
jqcc.extend(jqcc.cometchat, {
    crvariables : {themename: '<?php echo $theme;?>',
            ...
            chatroomBoxKeydown: function(event,chatboxtextarea,force) {
                ...
                if ((event.keyCode == 13 && event.shiftKey == 0) || force == 1 && !$(chatboxtextarea).hasClass('placeholder')) {
                    ...
                    if (condition) {
                        ...
                        if($('#cometchat_container_smilies').length == 1 && this.crvariables.mobileDevice != null){
            ...
            leaveChatroom: function(id, banflag, callbackfn) {
                ...
                <?php if (USE_COMET == 1 && COMET_CHATROOMS == 1):?>
                cometuncall_function(jqcc.cometchat.getChatroomVars('currentroom'));
                jqcc.cometchat.removeCRCSChannel(jqcc.cometchat.getChatroomVars('currentroom'));
                <?php endif;?>
                ...
                jqcc.ajax({
                    ...
                    data: {currentroom: this.crvariables.currentroom, flag:flag, basedata:this.crvariables.basedata},
                    type: 'post',
                    cache: false,
                    timeout: 10000,
                    success: function(data) {
                        if (data) {
                            ...
                            var key = '_'+data;
                            delete chatroomdata[key];
                            cc_chatroom.active = chatroomdata;
                            if(Object.keys(chatroomdata).length > 0){
                                var previousChatroomId = Object.keys(chatroomdata).reverse()[0].replace('_','');
                                cc_chatroom.open = previousChatroomId;
                            }
                            $.cookie('<?php echo $cookiePrefix;?>crstate', JSON.stringify(cc_chatroom), {path: '/'});
                            ...
                            if (typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].removeCurrentRoomTab) == "function")
                                jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].removeCurrentRoomTab(jqcc.cometchat.getChatroomVars('currentroom'));
                            if(Object.keys(chatroomdata).length > 0){
                                if (typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].switchChatroom) == "function" && callbackfn != 'mobilewebapp'){
                                ...
                                if (typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].loadLobby) == "function" && callbackfn != 'mobilewebapp'){
                                    jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].loadLobby(1);
                                }
            ...
            deleteChatroom: function(event,id){
                ...
                if (confirmDeletion == true) {
                    if(jqcc.cookie(this.crvariables.cookiePrefix+'crstate') != null) {
                        var cc_crstate = JSON.parse($.cookie(this.crvariables.cookiePrefix+'crstate'));
                        var currentroomid = cc_crstate.open;
            ...
            unbanChatroomUser: function(windowmode) {
            ...
            loadChatroomPro: function(uid,owner,longname,popoutmode) {
                ...
                if(this.crvariables.mobileDevice != null){
            ...
            silentroom: function(roomid, inviteid, roomname) {
                ...
                jqcc.cometchat.chatroom(roomid,roomname,1,inviteid,1,0,1);

            },
            checkChatroomPass: function(id,name,silent,password,clicked,type,encryptPass,force) {
                    ...
                    success: function(data) {
                        if (data) {
                            if (data['s'] != 'INVALID_PASSWORD' && data['s'] != 'BANNED' && data['s'] !='INVALID_CHATROOM' && data['s'] !='REQUIRED_PASSWORD') {
                                ...
                                if(force == 1) {
                                    if($.cookie('<?php echo $cookiePrefix;?>state')!=null) {
                                        var cc_state = $.cookie('<?php echo $cookiePrefix;?>state');
                                        var cc_states = cc_state.split(/:/);
                                        if(cc_states[5] == 'chatrooms'){
                                            var controlparameters = {"name":"active", "val":{}, "roomno":id, "messageCounter":0, "isOpen":1};
            ...
            chatroom: function(id,name,type,invite,silent,clicked,force) {
                if(id == null){
                    return;
                }
                ...
                if (this.crvariables.currentroom != id || force == 1) {
                    ...
                    jqcc.cometchat.checkChatroomPass(id,name,silent,this.crvariables.password,clicked,type,0,force);
            ...
            chatroomHeartbeat: function(forceUpdate) {
                if(forceUpdate != "undefined"  && typeof(forceUpdate) != "undefined") {
                    if(forceUpdate.toString().indexOf('^') > -1) {
                        ...
                        if(jqcc.cometchat.getChatroomVars('themename') == 'synergy'){
                        ...
                        if (data) {
                            ...
                            jqcc.each(data, function(type,item) {
                                ...
                                if (type == 'userid') {
                                    ...
                                    if (typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].createChatroomTab) == "function")
                                        jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].createChatroomTab(item);
                                }
                                if (type == 'chatrooms') {
                                    if (jqcc.cometchat.getChatroomVars('initializeAutoLogin') == 1 && jqcc.cometchat.getChatroomVars('themename') == 'synergy') {
                ...
                if(this.crvariables.initialize == 1){
                    ...
                    jqcc.cometchat.setChatroomVars('initialize',0);
                    if($.cookie(this.crvariables.cookiePrefix+'crstate') == null){
                        var cc_chatroom = {"active":{}, "open":""};
                        cc_chatroom = JSON.stringify(cc_chatroom);
                        $.cookie(this.crvariables.cookiePrefix+'crstate', cc_chatroom, {path: '/'});
                    }
                    var cc_chatroom = JSON.parse($.cookie(this.crvariables.cookiePrefix+'crstate'));
                    var chatroomData = cc_chatroom.active;
                    if(Object.keys(chatroomData).length > 0){
                    ...
                    if(IsJsonString(message)){
                        ...
                        switch(data[1]){
                            ...
                            case 'banned':
                                ...
                                break;
                            case 'deletemessage':
                                $("#cometchat_message_"+data[2]).remove();
                                return '';
                                break;
                            default :
                                break;
