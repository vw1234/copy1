...
(function($){
	...
	$.ccwriteboard = (function () {
        ...
        return {
			...
			getTitle: function() {
				return jqcc.ccwriteboard.getLanguage('title');
			},
			...
			init: function (params) {
				...
				if (currenttime-lastcall > 10) {
					...
					if(params.chatroommode == 1) {
						...
						$.getJSON(baseUrl+'plugins/writeboard/index.php?action=request&callback=?', {to: id, id: params.random, basedata: baseData});
					}
					if(mobileDevice){
						params.windowMode = 1;
					}
					jqcc.ccwriteboard.loadwriteboard(params);
				} else {
					alert(jqcc.ccwriteboard.getLanguage('wait_message'));
				}
			},
			...
			loadwriteboard: function(params){
				...
				if(typeof(params.chatroommode) != "undefined" && params.chatroommode==1){
					extraQueryString="&chatroommode=1&roomid="+params.to;
				}
				baseUrl = $.cometchat.getBaseUrl();
				baseData = $.cometchat.getBaseData();
				if(mobileDevice){
				...
				loadCCPopup(baseUrl+'plugins/writeboard/index.php?action=writeboard&type='+params.type+extraQueryString+'&id='+params.random+'&basedata='+baseData, 'writeboard',"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $writebWidth;?>,height=<?php echo $writebHeight;?>",width,height-50,jqcc.ccwriteboard.getLanguage('collaborative_document'),params.force,0,1,1,params.windowMode);
			}
        };
    })();
